...
across tests for which the null is rejected largely unequal to the true
value of \(\beta_1\). On the other hand, sample average of
\(\hat{\beta_1}\) on all data is approximately equal to the true value
of \(\beta_1\). The reason is the samples of \(\hat{\beta_1}\) which can
across tests for which the null is rejected are more likely to be
deviated from \(\beta_1 = 0\). In other word, the larger
\(\hat{\beta_1}\) are, the greater probability that they can reject
\(H_0: \hat{\beta_1} = 0\) which result in a large amount of sample
greater than \(\beta_1\). Also, when \(\beta_1 = 0\), the samples which
can reject \(H_0: \hat{\beta_1} = 0\) will be deviated from 0, which
means they will be significantly larger or smaller than 0, shown in the
plot above.
